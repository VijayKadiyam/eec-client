            ...
            <div class="col-md-6">
              ...
              <div class="card card-primary">
                <div class="card-header">
                  <h3 class="card-title">Edit records Details</h3>
                </div>
                <!-- /.card-header -->
                <!-- form start -->
                <div class="card-body">
...
export default {
  ...
  async asyncData({$axios, params}) {
    let records = await $axios.get(`/records/${params.id}`)
    return {
      form: records.data.data,
    }
  },
  data: () => ({
    loading: false
  ...
  methods: {
    async store() {
      try {
        this.loading = true
        let admin = await this.$axios.patch(`/records/${this.$route.params.id}`, this.form)
